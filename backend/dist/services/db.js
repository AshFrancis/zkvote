...
export function getEventsForDao(daoId, options = {}) {
    ...
    const countResult = database.prepare(countQuery).get(...countParams);
    return {
        events: events.map(e => ({
            id: e.id,
            dao_id: e.dao_id,
...
export function getDbStatus() {
    const database = initDb();
    const totalResult = database.prepare('SELECT COUNT(*) as total FROM events').get();
    const daoCountResult = database.prepare('SELECT COUNT(DISTINCT dao_id) as daoCount FROM events').get();
...
export function getAllCachedDaos() {
    const database = initDb();
    const rows = database.prepare('SELECT * FROM daos ORDER BY id ASC').all();
    return rows.map(row => ({
        id: row.id,
        name: row.name,
        creator: row.creator,
...
export function getCachedDaoCount() {
    const database = initDb();
    const result = database.prepare('SELECT COUNT(*) as count FROM daos').get();
    return result.count;
}
// ============================================
// MIGRATION FUNCTIONS
