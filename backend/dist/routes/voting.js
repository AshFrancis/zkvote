...
import { server, relayerKeypair, callWithTimeout, simulateWithBackoff, waitForTransaction, u256ToScVal, proofToScVal, scValToU256Hex, } from '../services/stellar.js';
...
router.post('/vote', authGuard, voteLimiter, validateBody(voteSchema), (async (req, res) => {
    ...
    try {
        ...
        const sendResult = await callWithTimeout(() => server.sendTransaction(preparedTx), 'send_vote');
...
router.get('/proposal/:daoId/:proposalId', queryLimiter, (async (req, res) => {
    ...
    try {
        ...
        log('error', 'proposal_fetch_error', { daoId, proposalId, error: err.message });
