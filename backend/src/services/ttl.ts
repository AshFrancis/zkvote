 ...
 */
async function submitCall(contractId: string, method: string, args: StellarSdk.xdr.ScVal[] = []): Promise<boolean> {
  try {
  ...
  try {
    ...
    for (const dao of daos) {
      ...
      if (config.daoRegistryContractId && isValidContractId(config.daoRegistryContractId)) {
        const ok = await submitCall(config.daoRegistryContractId, 'get_dao', [daoIdScVal]);
        if (ok) successCount++;
        else failCount++;
        await new Promise(resolve => setTimeout(resolve, 2000));
      }
      ...
      if (config.treeContractId && isValidContractId(config.treeContractId)) {
        const ok = await submitCall(config.treeContractId, 'current_root', [daoIdScVal]);
        if (ok) successCount++;
        else failCount++;
        await new Promise(resolve => setTimeout(resolve, 2000));
      }
      ...
      if (config.votingContractId && isValidContractId(config.votingContractId)) {
        const ok = await submitCall(config.votingContractId, 'proposal_count', [daoIdScVal]);
        if (ok) successCount++;
        else failCount++;
        await new Promise(resolve => setTimeout(resolve, 2000));
      }
    ...
    log('warn', 'ttl_renewal_dao_iteration_failed', { error: (err as Error).message });
  }
...
export function startTTLRenewal(intervalMs?: number): void {
  ...
  const interval = intervalMs ?? Number(process.env.TTL_RENEWAL_INTERVAL_MS || DEFAULT_TTL_RENEWAL_INTERVAL_MS);
