...
fn test_root_history_eviction_behavior() {
    ...
    assert_eq!(current_index, 34, "Current index should be 34 after 35 registrations (0-based)");
...
fn test_multiple_daos_separate_root_histories() {
    ...
    assert_eq!(idx_1, 9, "DAO 1 should have index 9 after 10 registrations (0-based)");
    assert_eq!(idx_2, 4, "DAO 2 should have index 4 after 5 registrations (0-based)");
    ...
    assert!(tree_client.root_ok(&dao_id_1, &root_1), "Root 1 valid in DAO 1");
    assert!(!tree_client.root_ok(&dao_id_2, &root_1), "Root 1 should NOT be valid in DAO 2");
...
fn test_tree_capacity_at_depth_5() {
    ...
    assert_eq!(final_idx, 31, "Should have index 31 after 32 registrations (0-based)");
